...
func GetOpenStackControlPlaneHandler(k8sClient *client.K8sClient) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if ok && name != "" {
			...
			if err != nil {
				return mcp.NewToolResultError(fmt.Sprintf("Failed to get OpenStackControlPlane: %v", err)), nil
			}
		} else {
			// List all OpenStackControlPlane CRs and return the first one
			controlPlanes, err := k8sClient.ListOpenStackControlPlanes(ctx, namespace)
			if err != nil {
				return mcp.NewToolResultError(fmt.Sprintf("Failed to list OpenStackControlPlanes: %v", err)), nil
			}
